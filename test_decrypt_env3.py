...
key_content = os.getenv("IDENTITY_MTLS_KEY_CONTENT")
private_key = serialization.load_pem_private_key(key_content.replace("\\n", "\n").encode('utf-8'), password=None)
encrypted_key = base64.b64decode(data["encryptedKey"])

for pad in [padding.PKCS1v15(), padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()), algorithm=hashes.SHA256(), label=None), padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA1()), algorithm=hashes.SHA1(), label=None)]:
    try:
