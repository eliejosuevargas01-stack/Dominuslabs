...
old_address_block = r'<div className="md:col-span-2">\s*<label className="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1\.5 flex items-center gap-1\.5">\s*<MapPin className="w-3\.5 h-3\.5 text-slate-400" /> Endereço Físico / Sede Operacional\s*</label>\s*<input\s*type="text"\s*value={settings\.address \|\| \'\'}\s*onChange={\(e\) => setSettings\({ \.\.\.settings, address: e\.target\.value }\)}\s*placeholder="Ex: Av\. das Nações Unidas, 12901 - Torre Leste, 18º andar - Brooklin, São Paulo/SP"\s*className="w-full px-4 py-2\.5 rounded-xl border border-slate-200 focus:border-purple-500 focus:ring-2 focus:ring-purple-200 outline-none text-sm transition-all"\s*/>\s*</div>'
