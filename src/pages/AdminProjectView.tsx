  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] gap-4">
        ...
        <p className="text-zinc-500 font-medium">Carregando detalhes do projeto...</p>
      </div>
    );
  }
  ...
  if (!project) {
    return (
      <div className="text-center py-12">
        ...
        <Link to="/project-hub" className="text-purple-600 font-bold hover:underline mt-4 inline-block">Voltar ao dashboard</Link>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 pb-12 animate-[fade-in_0.3s_ease-out]">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <Link to="/project-hub" className="inline-flex items-center gap-1.5 text-sm font-semibold text-zinc-500 hover:text-purple-600 transition-colors self-start">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="surface-card p-6 space-y-6">
            <div>
              ...
              <h1 className="text-3xl font-extrabold text-zinc-900 mt-3">{project.name}</h1>
              <p className="text-zinc-500 text-sm mt-1.5 font-medium">Cliente: {project.client_name}</p>
            </div>

            {project.description && (
              <div className="border-t border-zinc-100/80 pt-4">
                <h3 className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-2">Descrição / Escopo</h3>
                <p className="text-sm text-zinc-600 leading-relaxed break-words">{project.description}</p>
              </div>
            )}

            {/* Meta values list */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 border-t border-zinc-100/80 pt-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-zinc-400 text-[10px] font-bold uppercase tracking-wider">Investimento</p>
                  <p className="text-sm font-bold text-zinc-900">R$ {project.value.toLocaleString()}</p>
                </div>
              </div>

              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-lg bg-purple-50 text-purple-700 flex items-center justify-center border border-zinc-200/50">
                  <Calendar className="w-5 h-5" />
                </div>
                <div>
                  <p className="text-zinc-400 text-[10px] font-bold uppercase tracking-wider">Criado em</p>
                  <p className="text-sm font-semibold text-zinc-700">{new Date(project.created_at).toLocaleDateString()}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-zinc-400 text-[10px] font-bold uppercase tracking-wider">Última alteração</p>
                  <p className="text-sm font-semibold text-zinc-700">{new Date(project.updated_at).toLocaleDateString()}</p>
                </div>
              </div>
            </div>

            {/* Links integration */}
            <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-zinc-100/80">
              <div className="flex-1 p-3  border border-zinc-100 rounded-xl flex items-center justify-between">
                <div className="flex items-center gap-2.5 min-w-0">
                  <GithubIcon className="w-5 h-5 text-zinc-400 shrink-0" />
                  <div className="min-w-0">
                    <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">GitHub Repo</p>
                    {project.github_url ? (
                      <a href={project.github_url} target="_blank" rel="noopener noreferrer" className="text-xs font-semibold text-purple-600 hover:underline truncate block">
                        {project.github_url}
                      </a>
                    ) : (
              ...
              <div className="flex-1 p-3  border border-zinc-100 rounded-xl flex items-center justify-between">
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Deploy Preview</p>
                    {project.deploy_url ? (
                      <a href={project.deploy_url} target="_blank" rel="noopener noreferrer" className="text-xs font-semibold text-purple-600 hover:underline truncate block">
                        {project.deploy_url}
                      </a>
                    ) : (
                      <span className="text-xs text-zinc-400 italic">Não configurado</span>
            ...
            <div className="p-4 bg-amber-50/70 border border-amber-100 rounded-2xl flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <input
                  ...
                  className="w-full text-xs  border border-amber-200/60 rounded-lg px-2.5 py-1.5 focus:outline-none text-zinc-600 cursor-pointer font-mono mt-1"
                />
              </div>
            </div>

            {/* Webhook Github card */}
            <div className="p-4 bg-purple-50/70 border border-zinc-200 rounded-2xl flex items-center gap-3">
              <Sparkles className="w-5 h-5 text-purple-600 shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-[11px] font-bold text-purple-800 uppercase tracking-wider">Webhook de Integração GitHub (Exclusivo do Projeto)</p>
                <input
                  type="text"
                  readOnly
                  value={`${API_BASE_URL}/webhooks/github/${project.public_token}`}
                  onClick={(e) => (e.target as HTMLInputElement).select()}
                  className="w-full text-xs  border border-purple-200/60 rounded-lg px-2.5 py-1.5 focus:outline-none text-zinc-600 cursor-pointer font-mono mt-1"
                />
          ...
          <div className="surface-card p-6 space-y-4">
            <div className="flex items-center justify-between border-b border-zinc-100/80 pb-3 flex-wrap gap-2">
              <h2 className="text-lg font-bold text-zinc-900">Arquivos do Projeto (Assets)</h2>
              
              {!isViewer && (
                <label className="btn-secondary text-xs flex items-center gap-1.5 px-3 py-1.5 rounded-xl cursor-pointer hover:bg-zinc-100 transition-colors">
            ...
            {uploading && (
              <div className="flex items-center gap-2 text-purple-600 text-xs font-medium py-2">
            ...
            {assets.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-xs text-zinc-400 italic">Nenhum arquivo enviado para este projeto.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                {assets.map(asset => {
                  ...
                  return (
                    <div key={asset.id} className="p-3 bg-white border border-zinc-200/40 rounded-xl flex items-center gap-3 group relative hover:border-purple-300 transition-all">
                      <div className="p-2 bg-zinc-50 rounded-lg group-hover:bg-purple-50 transition-colors">
                        {getFileIcon(asset.file_type)}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-semibold text-zinc-800 truncate" title={asset.file_name}>
                          {asset.file_name}
                        </p>
                        <p className="text-[10px] text-zinc-400 mt-0.5">{formatBytes(asset.file_size)}</p>
                      </div>
                      <a 
                        href={absoluteUrl} 
                        target="_blank" 
        ...
        <div className="space-y-6">
          ...
          <div className="surface-card p-6 space-y-6">
            ...
            <div className="space-y-3">
              ...
              {commits.length === 0 ? (
                <p className="text-xs text-zinc-400 italic">Nenhum commit registrado ainda.</p>
              ) : (
                <ul className="space-y-2.5 max-h-[180px] overflow-y-auto pr-1">
                  {commits.map(c => (
                    <li key={c.id} className="p-2.5  border border-zinc-100 rounded-lg text-xs">
                      <div className="flex justify-between font-semibold text-zinc-700">
                        <span className="truncate max-w-[120px]">{c.author}</span>
                        <span className="text-zinc-400 font-mono text-[10px]">{c.commit_hash.substring(0, 7)}</span>
                      </div>
                      <p className="text-zinc-600 mt-1 break-words font-medium">{c.message}</p>
            ...
            <div className="space-y-3 border-t border-zinc-100/80 pt-4">
              ...
              {deploys.length === 0 ? (
                <p className="text-xs text-zinc-400 italic">Nenhum deploy registrado ainda.</p>
              ) : (
                <ul className="space-y-2.5 max-h-[180px] overflow-y-auto pr-1">
                  {deploys.map(d => (
                    <li key={d.id} className="p-2.5  border border-zinc-100 rounded-lg text-xs flex items-center justify-between gap-2">
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5">
                          <span className="font-semibold text-zinc-700 uppercase">{d.provider}</span>
                        ...
                        {d.deploy_url && (
                          <a href={d.deploy_url} target="_blank" rel="noopener noreferrer" className="text-[10px] text-purple-600 font-medium hover:underline block truncate mt-0.5">
                            {d.deploy_url}
                          </a>
                        )}
                      </div>
                      <span className="text-[9px] text-zinc-400 shrink-0 text-right">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4   animate-[fade-in_0.2s_ease-out]">
          <div className="bg-white rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl border border-zinc-100 flex flex-col max-h-[90vh] animate-[scale-up_0.25s_cubic-bezier(0.34,1.56,0.64,1)]">
            ...
            <div className="p-6 border-b border-zinc-100 flex items-center justify-between ">
              <div>
                <h3 className="text-xl font-bold text-zinc-900">Editar Detalhes do Projeto</h3>
                <p className="text-xs text-zinc-500 mt-0.5">Atualize as informações operacionais do projeto.</p>
              </div>
              <button 
            ...
            <form onSubmit={handleUpdateSubmit} className="flex-1 overflow-y-auto p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-zinc-500 uppercase tracking-wider">Nome do Projeto *</label>
                  <input
                    type="text"
                    name="name"
                    required
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-zinc-500 uppercase tracking-wider">Nome do Cliente *</label>
                  <input
                    type="text"
                    name="client_name"
                    required
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-zinc-500 uppercase tracking-wider">Tipo de Projeto</label>
                  <select
                    name="project_type"
                    value={editForm.project_type}
                    onChange={handleEditChange}
                    className="w-full text-sm border border-zinc-200 rounded-xl px-3.5 py-2.5 bg-white focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500"
                  >
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  onChange={handleEditChange}
                  className="w-full text-sm border border-zinc-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 resize-none"
                />
              </div>
