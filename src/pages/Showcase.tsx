...
export default function Showcase({ isDashboard = false }: { isDashboard?: boolean }) {
  ...
  if (loading) {
    return (
      <div className={isDashboard ? "flex flex-col items-center justify-center min-h-[300px]" : "flex flex-col items-center justify-center min-h-screen gap-4 bg-zinc-50 relative overflow-hidden"}>
        ...
        <Loader2 className="w-10 h-10 text-purple-600 animate-spin z-10" />
        <p className="text-zinc-500 font-medium z-10">Carregando cases de sucesso...</p>
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div className={isDashboard ? "flex flex-col items-center justify-center p-4" : "flex flex-col items-center justify-center min-h-screen p-4 bg-zinc-50 relative overflow-hidden"}>
        <div className="max-w-md w-full text-center surface-card p-8 border-rose-100 z-10">
          <FolderCheck className="w-12 h-12 text-rose-400 mx-auto mb-4" />
          <h3 className="text-lg font-bold text-zinc-800">Ops! Algo deu errado</h3>
          <p className="text-sm text-zinc-500 mt-2">{error || 'Erro ao carregar dados.'}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={isDashboard ? "relative z-10 w-full" : "min-h-screen  relative overflow-hidden flex flex-col justify-between"}>
      {/* Background Animation Bubbles */}
      {!isDashboard && (
        <div className="animated-bg">
          <div className="bg-bubble-1"></div>
      ...
      <div className={isDashboard ? "w-full space-y-6" : "max-w-6xl mx-auto w-full px-4 sm:px-6 lg:px-8 pt-8 relative z-10 flex-1"}>
        ...
        {!isDashboard && (
          <header className="flex items-center justify-between pb-12">
            <Link to="/" className="flex items-center gap-2 group">
              ...
              <span className="font-display font-extrabold text-2xl tracking-tight bg-gradient-to-r from-purple-800 via-indigo-700 to-emerald-600 bg-clip-text text-transparent">
                Dominuslabs
              </span>
            </Link>
            <span className="text-xs font-semibold px-3 py-1 rounded-full bg-purple-100/70 text-purple-800 border border-purple-200/40">
              Cases de Sucesso
            </span>
          </header>
        )}
        ...
        <section className="text-center max-w-3xl mx-auto space-y-4 pb-16">
          ...
          <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight text-zinc-900 leading-tight">
            Nossos Projetos e <span className="bg-gradient-to-r from-purple-700 to-indigo-600 bg-clip-text text-transparent">Cases de Sucesso</span>
          </h1>
        ...
        <section className="space-y-8">
          ...
          <div className="flex justify-center">
            <div className="bg-zinc-100 p-1.5 rounded-2xl flex gap-1 shadow-inner border border-zinc-200/40">
              <button
          ...
          {activeTab === 'completed' ? (
            completedProjects.length === 0 ? (
              <div className="text-center py-12 surface-card max-w-md mx-auto">
                <FolderCheck className="w-10 h-10 text-zinc-300 mx-auto mb-2" />
                <p className="text-xs text-zinc-400 italic">Nenhum projeto concluído no momento.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {completedProjects.map((proj, idx) => {
                  ...
                  return (
                    <div key={idx} className="surface-card flex flex-col justify-between group hover:border-purple-300 hover:shadow-lg transition-all relative overflow-hidden h-[450px]">
                      {/* Media Header Preview */}
                      <div className="h-48 w-full relative overflow-hidden bg-zinc-900 shrink-0">
                        {hasMedia ? (
                          isVideo ? (
                            ...
                            <div className="w-full h-full relative">
                              ...
                              {media.length > 1 && (
                                <span className="absolute bottom-3 right-3 text-[9px] font-bold px-2.5 py-0.5 rounded  backdrop-blur text-white uppercase tracking-wider">
                      ...
                      <div className="p-5 flex-1 flex flex-col justify-between">
                        <div className="space-y-2">
                          ...
                          <h3 className="text-base sm:text-lg font-bold text-zinc-800 tracking-tight line-clamp-1 group-hover:text-purple-700 transition-colors">
                            {proj.name}
                          </h3>
                        ...
                        <div className="space-y-3 mt-4">
                          <div className="pt-3 border-t border-zinc-100/50 flex items-center justify-between text-xs text-zinc-400 font-semibold">
                          ...
                          <div className="flex gap-2">
                            <button 
                              ...
                              className="flex-1 flex items-center justify-center gap-1.5 py-2.5 px-4 rounded-xl border border-zinc-200 text-purple-700 bg-purple-50/50 hover:bg-purple-600 hover:text-white hover:border-purple-600 transition-all font-bold text-xs cursor-pointer shadow-sm"
                            >
            ...
            ongoingProjects.length === 0 ? (
              <div className="text-center py-12 surface-card max-w-md mx-auto">
                ...
                <p className="text-xs text-zinc-400 italic">Nenhum projeto em desenvolvimento no momento.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  return (
                    <div key={idx} className="surface-card flex flex-col justify-between group hover:border-purple-300 hover:shadow-lg transition-all relative overflow-hidden h-[450px]">
                      ...
                      <div className="h-48 w-full relative overflow-hidden bg-zinc-900 shrink-0">
                        ...
                        {/* Status Badge */}
                        <span className="absolute top-3 right-3 text-[9px] font-extrabold px-2.5 py-0.5 rounded bg-purple-500 text-white uppercase tracking-wider">
                          Em Progresso
                        </span>
                      </div>
        ...
        <section className="pt-24 space-y-10">
          <div className="text-center max-w-md mx-auto space-y-2">
            <h2 className="text-2xl sm:text-3xl font-extrabold text-zinc-900 flex items-center justify-center gap-2">
          ...
          {data.testimonials.length === 0 ? (
            <div className="text-center py-12 surface-card max-w-md mx-auto">
              <Star className="w-10 h-10 text-zinc-300 mx-auto mb-2" />
              <p className="text-xs text-zinc-400 italic">Nenhum depoimento publicado ainda.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {data.testimonials.map((test, idx) => (
                <div key={idx} className="surface-card p-6 sm:p-8 space-y-6 flex flex-col justify-between hover:border-purple-200 hover:shadow-md transition-all relative">
                  ...
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-1">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <Star 
                          key={star} 
                          className={`w-4 h-4 ${star <= test.rating ? 'text-amber-400 fill-amber-400' : 'text-zinc-300'}`} 
                        />
                      ))}
                    </div>
                    <span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full bg-purple-50 text-purple-700 border border-zinc-200 uppercase tracking-wider">
                      {test.project_type}
                    </span>
                  </div>

                  {/* Comment */}
                  <blockquote className="text-sm sm:text-base text-zinc-600 font-medium italic leading-relaxed break-words flex-1 py-2">
                    "{test.comment}"
                  </blockquote>

                  {/* Client Metadata */}
                  <div className="border-t border-zinc-100/80 pt-4 flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-600 to-indigo-600 text-white flex items-center justify-center font-bold text-sm">
                      {test.client_name.charAt(0)}
                    </div>
                    <div>
                      <h4 className="text-sm font-bold text-zinc-800 leading-tight">{test.client_name}</h4>
                      <p className="text-[10px] text-zinc-400 font-semibold mt-0.5">Projeto: {test.project_name}</p>
                    </div>
                  </div>
                </div>
              ))}
      ...
      {selectedProject && (
        ...
        >
          <div 
            className="bg-white rounded-3xl w-full max-w-4xl overflow-hidden shadow-2xl border border-zinc-100 flex flex-col md:flex-row max-h-[90vh] md:max-h-[80vh] animate-[scale-up_0.25s_cubic-bezier(0.34,1.56,0.64,1)]"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Left Media Carrousel */}
            <div className="w-full md:w-1/2 bg-zinc-950 flex flex-col justify-between relative overflow-hidden h-[300px] md:h-auto min-h-[300px]">
            ...
            <div className="w-full md:w-1/2 p-6 sm:p-8 flex flex-col justify-between overflow-y-auto max-h-[50vh] md:max-h-full">
              <div>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-1">
                    <span className="text-[10px] font-extrabold px-2.5 py-0.5 rounded-full bg-purple-100 text-purple-800 border border-purple-200/50 uppercase tracking-wider inline-block">
                      {selectedProject.project_type}
                    </span>
                    <h3 className="text-xl sm:text-2xl font-extrabold text-zinc-900 tracking-tight mt-1.5">
                      {selectedProject.name}
                    </h3>
                  </div>
                  <button 
                    onClick={() => setSelectedProject(null)}
                    className="p-1.5 rounded-xl hover:bg-zinc-100 text-zinc-400 hover:text-zinc-700 transition-colors cursor-pointer shrink-0"
                  >
                    <X className="w-5 h-5" />
                  </button>
                </div>
                ...
                <div className="mt-6 space-y-3">
                  <h4 className="text-[11px] font-bold text-zinc-400 uppercase tracking-widest">Sobre o Case</h4>
                  <p className="text-sm text-zinc-600 leading-relaxed break-words font-medium whitespace-pre-line">
                  ...
                  <div className="mt-6 p-4 bg-amber-50/70 border border-amber-100/50 rounded-2xl space-y-3 relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-1">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <Star 
                          key={star} 
                          className={`w-3.5 h-3.5 ${star <= projectTestimonial.rating ? 'text-amber-400 fill-amber-400' : 'text-zinc-300'}`} 
                        />
                      ))}
                    </div>
                    <blockquote className="text-xs text-zinc-600 font-semibold italic leading-relaxed break-words">
                    ...
                    <div className="flex items-center gap-2.5 pt-1.5 border-t border-amber-100/40">
                      ...
                      <div>
                        <h5 className="text-xs font-bold text-zinc-800 leading-tight">{projectTestimonial.client_name}</h5>
                        <p className="text-[9px] text-zinc-400 font-semibold mt-0.5">Avaliação do Cliente</p>
                      </div>
                    </div>
                  </div>
                )}
              ...
              <div className="mt-8 pt-5 border-t border-zinc-100 flex items-center justify-between flex-wrap gap-4">
                <div className="flex items-center gap-4 text-xs font-semibold text-zinc-400">
                  <div>
                    <span className="block text-[9px] font-bold text-zinc-300 uppercase tracking-wider">Status</span>
                ...
                <div className="flex gap-2.5">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-1.5 bg-purple-600 hover:bg-purple-700 text-white px-5 py-2.5 rounded-xl font-bold text-xs shadow-md transition-all hover:scale-[1.01] active:scale-[0.99] cursor-pointer"
                  >
