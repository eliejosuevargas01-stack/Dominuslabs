...
export default function TaskChecklist({ projectId, admin, onTasksUpdated }: TaskChecklistProps) {
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-2 text-purple-600 text-sm py-4">
            ...
            return (
              <li
                ...
                className={`flex items-start gap-3 p-3 rounded-xl border transition-all duration-200 select-none ${
                  admin && !isGithubDone ? 'cursor-pointer hover:bg-purple-50/50' : 'cursor-not-allowed'
                } ${
                  isDone
                    ? ' border-slate-200/50 text-slate-400'
                    : 'bg-white border-zinc-200/50 text-slate-700 shadow-sm'
                }`}
                title={isGithubDone ? "Esta tarefa foi concluída via commit no GitHub e não pode ser alterada." : ""}
              >
                <div className="mt-0.5">
                  {isDone ? (
                    <CheckCircle2 className={`w-5 h-5 ${isGithubDone ? 'text-purple-500 fill-purple-50' : 'text-emerald-500 fill-emerald-50'}`} />
                  ) : (
                    <Circle className={`w-5 h-5 ${admin ? 'text-purple-300 hover:text-purple-600' : 'text-slate-300'}`} />
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  {isGithubDone && (
                    <span className="block text-[10px] font-bold text-purple-600 mt-0.5 animate-pulse">
      ...
      {admin && (
        <form onSubmit={handleAddTask} className="flex gap-2 mt-2">
          <input
            ...
            className="flex-1 text-sm  border border-zinc-200/60 rounded-xl px-3.5 py-2 focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500"
            disabled={addingTask}
          />
          <button
            type="submit"
