...
def create_task(project_id: int, task_in: ProjectTaskCreate, db: Session = Depends(get_db), current_user: str = Depends(check_project_edit_permission)):
    ...
    """
# Lógica de decisão (if): Avalia 'if task_in.project_id != proje...' para garantir que a regra de negócio siga o fluxo correto ou evite erros (ex: validação de unicidade ou estado).
...
# Lógica de decisão (if): Avalia 'if not task:...' para checar condições e aplicar restrições de permissão/acesso aos dados do usuário.
    if not task:
        raise HTTPException(status_code=404, detail="Task not found")
        
    # Block undoing tasks completed via GitHub Webhook
# Lógica de decisão (if): Avalia 'if task.completed_by_github an...' para garantir que a regra de negócio siga o fluxo correto ou evite erros (ex: validação de unicidade ou estado).
...
# Lógica de decisão (if): Avalia 'if project.status != ProjectSt...' para garantir que a regra de negócio siga o fluxo correto ou evite erros (ex: validação de unicidade ou estado).
...
def get_public_showcase(db: Session = Depends(get_db)):
    ...
    projects_list = []
# Lógica de repetição (for): Itera sobre elementos de 'for p in all_project...' processando múltiplos dados em lote para as regras de domínio.
    ...
    testimonials_list = []
# Lógica de repetição (for): Itera sobre elementos de 'for f in feedbacks:...' processando múltiplos dados em lote para as regras de domínio.
    for f in feedbacks:
