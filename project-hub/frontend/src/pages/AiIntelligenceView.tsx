...
export default function AiIntelligenceView({
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-12">
      ...
      {/* Medidor de Franquia */}
      <div className="bg-white/90 backdrop-blur-md p-6 rounded-2xl border border-violet-100 shadow-sm space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-xs font-bold">
                <span className="text-slate-700">Tokens Processados</span>
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-xs font-bold">
                <span className="text-slate-700">Mensagens Enviadas</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="p-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-9 pr-3 py-1.5 text-xs rounded-xl border border-slate-200 focus:border-purple-500 outline-none w-full sm:w-64"
                />
              </div>
            </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-bold text-slate-900 text-sm">{log.clienteNome}</span>
                        <span className="text-[11px] text-slate-400">{log.dataHorario}</span>
                      </div>
                      <p className="text-xs text-slate-600 line-clamp-1 mb-2">{log.resumoIntencao}</p>

                      <div className="flex items-center justify-between text-[11px]">
                        <span className="text-slate-400 font-mono">{log.totalTokens} tokens</span>
        ...
        {selectedLog && (
          <div className="lg:col-span-6 bg-white/90 backdrop-blur-md rounded-2xl border border-violet-100 shadow-sm flex flex-col h-[580px]">
            {/* Panel Header */}
            <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 rounded-t-2xl">
              <div>
                <h3 className="font-bold text-slate-900 text-sm">{selectedLog.clienteNome}</h3>
                <p className="text-xs text-slate-500">{selectedLog.telefone}</p>
              </div>
              <button
                  ...
                  >
                    <div
                      className={`w-7 h-7 rounded-full flex items-center justify-center text-xs flex-shrink-0 ${
                        isIa ? 'bg-purple-600 text-white' : 'bg-slate-200 text-slate-700'
                      }`}
                    >
