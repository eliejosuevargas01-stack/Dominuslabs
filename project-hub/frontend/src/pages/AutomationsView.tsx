          ...
          {localRules.length > 0 ? (
            localRules.map((rule) => {
              ...
              return (
                <div
                  key={rule.id}
                  className="p-6 flex items-center justify-between gap-4 hover:bg-slate-50/60 transition-colors"
                >
                  <div className="space-y-1 max-w-3xl">
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-slate-900 text-sm">{rule.titulo}</span>
                      {rule.categoria && (
                    ...
                    <p className="text-xs text-slate-500 leading-relaxed">{rule.descricao}</p>
                  </div>
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-7 w-12 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none disabled:opacity-50 ${
                      rule.ativo ? 'bg-purple-600' : 'bg-slate-300'
                    }`}
                  >
                    <span
                    ...
                    >
                      {isBusy ? (
                        ...
                        <Power className={`w-3 h-3 ${rule.ativo ? 'text-purple-600' : 'text-slate-400'}`} />
                      )}
                    </span>
                  </button>
                </div>
            ...
            <div className="py-16 text-center text-slate-400">
              ...
              <p className="text-sm font-semibold text-slate-600">Nenhuma regra de automação mapeada.</p>
              <p className="text-xs text-slate-400 mt-1">Conecte o array de regras vindo do back-end (GET /api/v1/automations/rules).</p>
            </div>
          )}
        </div>
      </div>
